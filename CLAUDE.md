...
- **ARO messages are terse.** LLM-to-LLM messages in an ARO are one or two lines. No framing, no restating the scope, no recap of decisions already in the thread. `claim:` / `done:` / `audit: pass|pass with nit X|fail Y` / `ship:` each fit on a single line. Every word must earn its place — token cost is multiplied across all members. Exception: when Elazar asks for a report or analysis, that request overrides terseness for that one message.
